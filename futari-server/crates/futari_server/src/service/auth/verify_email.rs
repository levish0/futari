...
pub async fn issue_pending_email_signup_token(
    ...
    let email_key = futari_constants::email_signup_email_key(&signup_data.email);
    let handle_key = futari_constants::email_signup_handle_key(&signup_data.handle);
    let token_key = futari_constants::email_verification_key(&token);
...
pub async fn find_pending_email_signup_by_email(
    ...
    find_pending_email_signup_by_index(redis_conn, &futari_constants::email_signup_email_key(email))
        .await
}
...
pub async fn delete_pending_email_signup_indices(
    ...
    delete_key(
        redis_conn,
        &futari_constants::email_signup_email_key(&signup_data.email),
    )
    .await?;
    delete_key(
        redis_conn,
        &futari_constants::email_signup_handle_key(&signup_data.handle),
    )
    .await?;
...
pub async fn get_pending_signup_remaining_minutes(
    ...
    let key = futari_constants::email_verification_key(token);
...
async fn find_pending_email_signup_by_index(
    ...
    let verification_key = futari_constants::email_verification_key(&token);
